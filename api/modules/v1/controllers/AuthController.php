...
class AuthController extends Controller
{
    ...
    public function actionBuildTokenFromCf()
    {
        ...
        if (!$token) ResponseError::returnSingleError(403, "Token non valido");

        $cf = $token->getClaim('cf');
        if (empty($cf)) ResponseError::returnSingleError(404, "Codice fiscale non trovato");
        ...
        $utente_app = ViewUtentiApp::find()
            ...
            ->all();
        if (count($utente_app) == 0) ResponseError::returnSingleError(404, "Utente non abilitato");
        ...
        if (count($valid_users) == 0) ResponseError::returnSingleError(404, "Non sei abilitato all'uso dell'app");

        $ana = UtlAnagrafica::findOne(['codfiscale' => strtoupper($cf)]);
        if (!$ana) ResponseError::returnSingleError(404, "Dati anagrafici non trovati, non puoi accedere");

        $response_users = [];
        foreach ($valid_users as $_user) {

            $utente = UtlUtente::findOne(['id' => $_user->id_utl_utente]);
            if (!$utente || empty($_user->id_user)) ResponseError::returnSingleError(422, "Non sei abilitato all'uso dell'app");
            ...
            $response_users[] = [
                ...
                'rappresentante_legale' => $_user->rappresentante_legale,
    ...
    private function buildUserByProfile(ViewUtentiApp $view_utente)
    {
        ...
        try {
            ...
            if (in_array($view_utente->tipo_utente_app, ['VOLONTARIO', 'OPERATORE PC']) && !empty($view_utente->id_volontario)) {
                ...
                if (!$volontario) throw new \Exception("Volontario non presente", 1);
                ...
                if (empty($view_utente->iduser)) {
                    ...
                    if (!$user->save()) throw new \Exception(json_encode($user->getErrors()), 1);
                }
    ...
    public function actionLogin()
    {
        ...
        if ($model->load($params, '') && $model->login()) :
            ...
            if (!$_utente_app) ResponseError::returnSingleError(404, "Utente non abilitato");
            ...
            if ($postData['tipo_organizzazione'] == 1) {
                ...
                if (!$volontario) {
                    ...
                    $utente_app_gia_registrato = ViewUtentiApp::find()
                        ...
                        ->one();
                    if ($utente_app_gia_registrato) ResponseError::returnSingleError(422, "Utente già registrato");
                } else {
