...
class AttivazioniController extends ActiveController
{
    ...
    public function actionView($id) {
        ...
        if($attivazione->rl_to_check == 1) {
            ...
            if(!$attivazione->save()) ResponseError::returnMultipleErrors(422, $attivazione->getErrors());
        }
    ...
    private function getAvaibleResources($attivazione, $utente_app) {
        ...
        if(!empty( $attivazione->idautomezzo ) ) {
            ...
            if(!empty($attivazione->idsede)) $risorse_q->andWhere(['idsede'=>$attivazione->idsede]);
    ...
    public function actionFeedback($id) {
        ...
        try {
            ...
            if(!empty($attivazione->feedbackRl)) ResponseError::returnSingleError(422, "Feedback già fornito");
            ...
            if(Yii::$app->request->post('action') == 'confirm') {
                ...
                if(empty(Yii::$app->request->post('motivazione_rifiuto')) || !in_array(Yii::$app->request->post('motivazione_rifiuto'), array_keys(UtlIngaggioRlFeedback::getMotivazioniRifiuto()))) ResponseError::returnSingleError(422, "Inserisci le motivazioni del rifiuto");
            ...
            if(!$feed->save()) ResponseError::returnMultipleErrors(422, $feed->getErrors());
    ...
    private function isServiceAvaible() {
        ...
        if(!$_utente_app || 
            ...
            $_utente_app->rappresentante_legale != 1 ) ResponseError::returnSingleError(404, "Utente non abilitato");
    ...
    private function canViewAttivazione(UtlIngaggio $attivazione, $utente_app) {
        ...
        if($attivazione->idorganizzazione != $utente_app->id_organizzazione) ResponseError::returnSingleError(422, "Non puoi visualizzare questa attivazione");
    ...
    private function canEditAttivazione(UtlIngaggio $attivazione, $utente_app) {
        ...
        if(
            $attivazione->idorganizzazione != $utente_app->id_organizzazione ||
