...
class AtomController extends ActiveController
{
    ...
    /**
     * Prendi gli stubs
     * @todo eliminare la tabella tab_macrotipologia
    ...
    public function actionIndex() {
        ...
        if(isset($_SERVER['X-Forwarded-Proto'])) $pageURL = $_SERVER['X-Forwarded-Proto'];
        ...
        foreach ($messages as $message) {
            ...
            if(!is_bool($pub)) $pub->setTimezone(new \DateTimeZone('Europe/Rome'));
            $exp = \DateTime::createFromFormat('Y-m-d H:i:sP', $message->expires.":00");
            if(!is_bool($exp)) $exp->setTimezone(new \DateTimeZone('Europe/Rome'));
