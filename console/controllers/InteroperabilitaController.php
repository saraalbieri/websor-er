...
class InteroperabilitaController extends Controller
{
    ...
    public function actionLogin($debug = 0)
    {
        ...
        if($debug == 1) $this->stdout(var_dump($res), Console::BOLD);
    }
            ...
            if (!empty($res['data']['list'])) {
                foreach ($res['data']['list'] as $key => $item) {
                    ...
                    try {
                        ...
                        $odv['VolSede']['indirizzo'] = !empty($item['indirizzo_sede']) ? $item['indirizzo_sede'] : "INDIRIZZO NON DEFINITO";
    ...
    public function actionGetVolontario($commit = 0, $debug = 0)
    {
        ...
        for ($i = 1; $i <= $pageCount; $i++) {
            ...
            if($debug == 1) echo "RICHIAMO PAGINA " . $i . "\n";
            $res = MgoHttpServices::getVolontario($i);
            if($debug == 1) echo "RICEVUTO RISULTATO PAGINA " . $i . "\n";
                ...
                foreach ($res['data']['list'] as $item) {
                    ...
                    try {
                        ...
                        if (empty($anagrafica)) {
                            ...
                            if(empty($anagrafica)) $anagrafica = new UtlAnagrafica();
                        }
                        ...
                        if (isset($item['specializzazione']) && !empty($item['specializzazione'])) {
                            ...
                            foreach ($item['specializzazione'] as $key => $specializzazione) {
                                ...
                                if ($specializzazioneMd->save()) {
                                    ...
                                    if(!$has) $volontario->link('specializzazione', $specializzazioneMd);
                                    
                                    if($this->debug == 1) $this->stdout("\nSpecializzazione SAVED", Console::FG_GREEN);
    ...
    private function upsertContattoOrganizzazione($organizzazione, $item)
    {
        ...
        $conOrgContattoArray = ConOrganizzazioneContatto::find()->where(['id_organizzazione' => $organizzazione->id])->indexBy('id_sync')->asArray()->all();

        // Creo $arrayDeletedItems array di appoggio per gli item cancellati
        $arrayCheckDeletedItems = [];
        ...
        foreach ($item['contatti_odv'] as $key => $contatto) {
            ...
            if (empty($conOrgContattoArray)) {
                ...
                $contatto['id_sync'] = 'MGO_' . $contatto['id_contatto'];
                if ($mdContatto->load(['UtlContatto' => $contatto]) && $mdContatto->save()) {
                    ...
                    if($this->debug == 1) $this->stdout("\nContatto NOT SAVED", Console::FG_RED);
                ...
                $checkIndex = 'MGO_' . $contatto['id_connessione'];
                if (empty($conOrgContattoArray[$checkIndex])) {
                    ...
                    if ($mdContatto->load(['UtlContatto' => $contatto]) && $mdContatto->save()) {
                        ...
                        if($this->debug == 1) $this->stdout("\nUpsert Contatto NOT SAVED", Console::FG_RED);
                        AppSyncErrorLog::createError(
                            'odv', 
                    ...
                    $arrayCheckDeletedItems[$checkIndex] = $mdContatto;
                }
            }
        }
        ...
        foreach ($conOrgContattoArray as $key => $insertedContatto) {
            if (!empty($arrayCheckDeletedItems) && empty($arrayCheckDeletedItems[$key])) {
                // Rimuovo la relazione
                $organizzazione->unlink('contatto', $arrayCheckDeletedItems[$key]);
            }
        }
    }
    ...
    private function upsertContattoSede($sede, $item)
    {
        ...
        $conSedeContattoArray = ConSedeContatto::find()->where(['id_sede' => $sede->id])->indexBy('id_sync')->asArray()->all();
        ...
        foreach ($item['contatti_sede'] as $key => $contatto) {
            ...
            if (empty($conSedeContattoArray)) {
                ...
                if ($mdContatto->load(['UtlContatto' => $contatto]) && $mdContatto->save()) {
                    ...
                    if($this->debug == 1) $this->stdout("\nContatto sede NOT SAVED", Console::FG_RED);
                    AppSyncErrorLog::createError(
                            'odv', 
                ...
                if (empty($conSedeContattoArray[$checkIndex])) {
                    ...
                    if ($mdContatto->load(['UtlContatto' => $contatto]) && $mdContatto->save()) {
                        ...
                        if($this->debug == 1) $this->stdout("\nUpsert Contatto Sede NOT SAVED", Console::FG_RED);
                        AppSyncErrorLog::createError(
                            'odv', 
        ...
        foreach ($conSedeContattoArray as $key => $insertedContatto) {
            if (!empty($arrayCheckDeletedItems) && empty($arrayCheckDeletedItems[$key])) {
                // Rimuovo la relazione
                $sede->unlink('contatto', $arrayCheckDeletedItems[$key]);
            }
        }
    }
    ...
    private function upsertSpecializzazioni($organizzazione, $item)
    {
        ...
        if (!empty($item['specializzazioni'])) {
            ...
            $organizzazione->unlinkAll('sezione-specialistica');
            ...
            foreach ($item['specializzazioni'] as $key => $spec) {
                ...
                if ($sezioneSpecialistica->save()) {
                    ...
                    $organizzazione->link('sezione-specialistica', $sezioneSpecialistica);
