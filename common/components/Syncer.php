...
class Syncer {
    ...
    private function syncOrgSezioneSpecialistica($org, $spec)
    {
        ...
        foreach ($spec as $s) {
            ...
            if(!$s_obj) {
                ...
                $s_obj->load( $this->objectMapping( $s, 'sezione-specialistica') );
                if(!$s_obj->save()) {
                    Yii::error($s_obj->getErrors());
                    //throw new \Exception("Errore salvataggio ".json_encode($s_obj->getErrors()));
                } else {
    ...
    private function org_update() {
        ...
        if(!empty($this->message['data']['volOrganizzazione'][0]['sezione-specialistica'])) :
            
            $this->syncOrgSezioneSpecialistica($org, $this->message['data']['volOrganizzazione'][0]['sezione-specialistica']);
        else: 
    ...
    private function update_attrezzatura()
    {
        ...
        if(!isset($attrezzatura_data['UtlAttrezzatura']['modello'])) $attrezzatura_data['UtlAttrezzatura']['modello'] = '-';
    ...
    private function sezioneSpecialistica_updated()
    {
        ...
        $t->load($this->objectMapping($this->message['data'], 'sezione-specialistica'));
        if(!$t->save()) {
            Yii::error($t->getErrors());
            throw new \Exception("Errore salvataggio ".json_encode($t->getErrors()));
        }
