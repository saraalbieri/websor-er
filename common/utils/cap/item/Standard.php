...
class Standard implements ItemInterface {
	...
	protected function setInfo($info, $n) {
		...
		foreach ((array) $info as $key => $value) {
			switch($key) {
				...
				break;
				case 'category':
					if(!isset($this->info[$n]['categories']) || empty((array) $this->info[$n]['categories'])) $this->info[$n]['categories'] = [];
				...
				break;
				case 'responseType':
					if(!isset($this->info[$n]['responseType']) || empty((array) $this->info[$n]['responseType'])) $this->info[$n]['responseType'] = [];
				...
				break;
				case 'eventCode':
					if(!isset($this->info[$n]['eventCodes'])) $this->info[$n]['eventCodes'] = [];
					if(is_array($value)){
						foreach ($value as $v) {
				...
				break;
				case 'parameter':
					if(!isset($this->info[$n]['parameters'])) $this->info[$n]['parameters'] = [];
				...
				break;
				case 'effective': case 'onset': case 'expire': case 'expires':
					if(is_string($value)) $this->info[$n][$key] = new \DateTime($value);
				break;

				case 'resource':
					if(!isset($this->info[$n]['resources'])) $this->info[$n]['resources'] = [];

					$value = (array) $value;
					$this->info[$n]['resources'][] = [
		...
		if(!isset($this->info[$n]['language'])) $this->info[$n]['language'] = 'en-US';
	...
	protected function setPosition($info, $n) {
		...
		foreach ($area as $key => $value) {
			switch($key) {
                ...
                break;
                case 'geocode':
                	if(!isset($this->info[$n]['geocodes'])) $this->info[$n]['geocodes'] = [];
	...
	protected function setGeometry($i) {
		...
		if($n == 1) $this->info[$i]['geometry'] = $this->info[$i]['geometries'][0];
	}
	...
	protected function setCentroid($n) {
		if(empty($this->info[$n]['geometry']) || $this->info[$n]['geometry'] == '') return;
