...
class EventoController extends Controller
{
    ...
    public function actionCreateTaskMattinale()
    {
        ...
        if ($taskEvento->load(Yii::$app->request->post())) {
            ...
            if (!$taskEvento->save()) throw new \Exception(json_encode($taskEvento->getErrors()), 1);
    ...
    public function actionUpdateElicottero($id)
    {
        ...
        try {
            ...
            if ($elicottero->deleted == 1 && Yii::$app->request->method == 'POST') throw new Exception("Richiesta annullata, non si può modificare", 1);
            ...
            if (
                !empty($data['RichiestaElicottero']['date']) ||
            ...
            if (
                !empty($data['RichiestaElicottero']['date_arrivo_stimato']) ||
            ...
            if (
                !empty($data['RichiestaElicottero']['date_atterraggio']) ||
    ...
    {
        $elicottero = RichiestaElicottero::findOne($id);
        if (!$elicottero) throw new \yii\web\HttpException(422, "Richiesta non trovata");

        $elicottero->deleted = 1;
        if (!$elicottero->save()) Yii::$app->session->setFlash('error', $elicottero->getErrors());
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            try {
                ...
                if (!empty($model->idparent)) \common\models\cap\CapExposedMessage::generateFromEvent(UtlEvento::findOne($model->idparent), 'Added fronte');
        ...
        if (!empty($postData)) {
            ...
            try {
                ...
                if (!empty($postData['UtlEvento']['saleOperativeEsterne'])) {
                    ...
                    if (!$capExposedMessage) throw new \Exception('Nessun messaggio cap trovato per questo evento');
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            try {
                ...
                if (
                    isset($posted['UtlEvento']['stato']) && $posted['UtlEvento']['stato'] == 'Chiuso' &&
                ...
                if (
                    isset($posted['UtlEvento']['stato']) && $old_stato == 'Chiuso' &&
                    $posted['UtlEvento']['stato'] != $old_stato
                ) :
                ...
                if (
                    !empty($old_parent) && !empty($model->idparent) && $old_parent != $model->idparent
                ) {
                ...
                if (
                    empty($old_parent) && !empty($model->idparent)
                ) {
                    $fronti = UtlEvento::find()
                        ->where(['!=', 'stato', 'Chiuso'])
                        ->andWhere(['idparent' => $model->id])->count();

                    if ($fronti > 0) throw new \Exception("Non puoi trasformare questo evento in un fronte, ha dei fronti associati", 1);
