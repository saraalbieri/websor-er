...
class ReportController extends Controller
{
    ...
    private function buildFilters( $params_map, $model = null )
    {
        ...
        //var_dump($filter_model->getAttributes());
    ...
    public function actionEventi()
    {
        ...
        */
        ...
        //print("<pre>" . print_r($provider,true) . "</pre>"); die();
    ...
    public function actionCoau()
    {
        ...
        $command = $connection->createCommand($q);
        //var_dump($filters['filter_params']); die();
        if ( !empty($filters['filter_params']) ) {
            $command->bindValues($filters['filter_params']);
        }
        ...
        foreach ($result as $row_q) {
            ...
            $n++;
            /*
            if(empty($row_q['data_richieste'])) {
                $indices[$row_q['provincia']] = $n;
                $data[] = $row_q;
                ...
                // Per inserimento negli expand
                
                if(!isset($data[$indices[$row_q['provincia']]]['children'])) $data[$indices[$row_q['provincia']]]['children'] = [];
                $data[$indices[$row_q['provincia']]]['children'][] = $row_q;
            }*/
        }
        ...
        foreach ($result as $row_q) { 
            ...
            if(empty($row_q['giorno'])) {
                ...
                if(!isset(
                    ...
                    //echo "setto\n";
                    $final[$row_q['anno'] . "_" . $row_q['mese'] . "_"]['children'] = [];
                    //var_dump($final[$row_q['anno'] . "_" . $row_q['mese'] . "_"]);
                }

                //$final[$row_q['anno'] . "_" . $row_q['mese'] . "_"]
