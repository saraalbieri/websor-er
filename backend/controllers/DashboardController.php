...
class DashboardController extends Controller
{
    ...
    public function actionFineGiornata()
    {
        /*
            ...
            Query
            SELECT t.tipologia, count(e.id) FILTER (WHERE e.stato <> 'Chiuso') 
            FROM utl_tipologia t 
            RIGHT JOIN utl_evento e ON e.tipologia_evento = t.id 
            WHERE t.idparent is null
            GROUP BY t.id
            ORDER BY t.tipologia ASC;

            SELECT p.sigla, count(e.id) FILTER (WHERE e.stato <> 'Chiuso' AND c.id_provincia = p.id) 
            FROM loc_provincia p
            RIGHT JOIN utl_evento e ON e.stato <> 'Chiuso'
            LEFT JOIN loc_comune c ON c.id = e.idcomune
            WHERE p.id_regione = ".intval(Yii::$app->params['region_filter_id'])."
            GROUP BY p.id
            ORDER BY p.sigla ASC;

            SELECT count(*), 'today' as ref
            FROM utl_evento 
            WHERE dataora_evento::date = CURRENT_DATE
            UNION 
            SELECT count(*), 'yesterday' as ref
            FROM utl_evento 
            WHERE dataora_evento::date = (NOW() - INTERVAL '1 DAY')::date
            ;

            SELECT count(model_id), 'today' as ref
            FROM tbl_audit_trail 
            WHERE new_value = 'Chiuso' AND old_value <> 'Chiuso' AND action = 'CHANGE' AND model = 'common\models\UtlEvento' AND stamp::date = CURRENT_DATE
            UNION 
            SELECT count(model_id), 'yesterday' as ref
            FROM tbl_audit_trail 
            WHERE new_value = 'Chiuso' AND old_value <> 'Chiuso' AND action = 'CHANGE' AND model = 'common\models\UtlEvento' AND stamp::date = (NOW() - INTERVAL '1 DAY')::date;
            ...
            LEFT JOIN utl_evento e ON e.sottotipologia_evento = t.id 
            ...
            --1=Comune
            --2=VVF
            SELECT g.descrizione, count(e.id) FILTER (WHERE e.stato <> 'Chiuso') 
            FROM evt_gestore_evento g
            RIGHT JOIN utl_evento e ON e.id_gestore_evento = g.id
            LEFT JOIN utl_tipologia t ON t.id = e.tipologia_evento
            WHERE t.tipologia ilike 'incendio'
            GROUP BY g.id
            ORDER BY g.descrizione ASC;

            SELECT p.sigla, count(e.id) FILTER (WHERE e.stato <> 'Chiuso' AND c.id_provincia = p.id AND t.tipologia ilike 'incendio') 
            FROM loc_provincia p
            RIGHT JOIN utl_evento e ON e.stato <> 'Chiuso'
            LEFT JOIN utl_tipologia t ON t.id = e.tipologia_evento
            LEFT JOIN loc_comune c ON c.id = e.idcomune
            ...
            SELECT p.sigla, count(e.id) FILTER (WHERE e.stato <> 'Chiuso' AND c.id_provincia = p.id AND t.tipologia ilike 'incendio' AND m.tipologia ilike 'boschivo') 
            FROM loc_provincia p
            RIGHT JOIN utl_evento e ON e.stato <> 'Chiuso'
            LEFT JOIN utl_tipologia t ON t.id = e.tipologia_evento
            ...
            SELECT p.sigla, count(i.id) FILTER (WHERE i.created_at::date = CURRENT_DATE AND c.id_provincia = p.id AND 
                (t.descrizione ilike 'autobotte' OR t.descrizione ilike 'pickup')
            ) 
            FROM loc_provincia p
            RIGHT JOIN utl_ingaggio i ON i.id is not null
            ...
            SELECT COALESCE( sum(n_lanci), 0) as numero_lanci FROM richiesta_elicottero WHERE created_at::date = CURRENT_DATE AND engaged = true;

            SELECT 
             COALESCE( sum( (SELECT v.stop_local_timestamp - v.start_local_timestamp) ), '00:00:00'::interval) as ore_di_volo
             FROM utl_arka_voli v
              WHERE v.start_local_timestamp::date = CURRENT_DATE;

         */
        
        

        return $this->render('fine_giornata', 
    ...
    public function actionExportExcel() {
        ...
        /*
        'Eventi aperti il ' . $today->format('d/m/Y') . ' / tipologia'
'Eventi aperti il ' . $today->format('d/m/Y') . ' / provincia'
'Incendi aperti il ' . $today->format('d/m/Y') . ' / sottotipologia'
'Incendi aperti il ' . $today->format('d/m/Y') . ' / ente gestore'
'Incendi aperti il ' . $today->format('d/m/Y') . ' / provincia'
'Incendi boschivi (' . $today->format('d/m/Y') . ') / provincia'
         */
    ...
    public function actionExportPdf() {
        ...
        $total = 0;
        foreach ($data['eventi_attivi_tipologia'] as $row) $total += $row['conteggio'];
        if($total > 0) {
            $graph = new Graph\PieGraph(400, 300);
            //$graph->title->Set("EVENTI ATTIVI PER TIPOLOGIA");
        
            $graph->SetBox(true);
            $pie_data = array_map(function($e){ return $e['conteggio'];}, $data['eventi_attivi_tipologia']);
        ...
        $total = 0;
        foreach ($data['eventi_attivi_provincia'] as $row) $total += $row['conteggio'];
        if($total > 0) {
            $graph = new Graph\PieGraph(400, 300);
            ...
            $pie_data = array_map(function($e){ return $e['conteggio'];}, $data['eventi_attivi_provincia']);
            $p1 = new Plot\PiePlot($pie_data);
            $p1->value->setFormat('%01.2f%%');
            $p1->ShowBorder();
            $p1->SetColor('black');
        ...
        $total = 0;
        foreach ($data['incendi_attivi_tipologia'] as $row) $total += $row['conteggio'];
        if($total > 0) {
            $graph = new Graph\PieGraph(400, 300);
            ...
            $pie_data = array_map(function($e){ return $e['conteggio'];}, $data['incendi_attivi_tipologia']);
            $p1 = new Plot\PiePlot($pie_data);
            $p1->value->setFormat('%01.2f%%');
            $p1->ShowBorder();
            $p1->SetColor('black');
        ...
        $total = 0;
        foreach ($data['incendi_attivi_ente_gestore'] as $row) $total += $row['conteggio'];
        if($total > 0) {
            $graph = new Graph\PieGraph(400, 300);
            ...
            $pie_data = array_map(function($e){ return $e['conteggio'];}, $data['incendi_attivi_ente_gestore']);
            $p1 = new Plot\PiePlot($pie_data);
            $p1->value->setFormat('%01.2f%%');
            $p1->ShowBorder();
            $p1->SetColor('black');
        ...
        $total = 0;
        foreach ($data['incendi_provincia'] as $row) $total += $row['conteggio'];
        if($total > 0) {
            $graph = new Graph\PieGraph(400, 300);
            ...
            $pie_data = array_map(function($e){ return $e['conteggio'];}, $data['incendi_provincia']);
            $p1 = new Plot\PiePlot($pie_data);
            $p1->value->setFormat('%01.2f%%');
            $p1->ShowBorder();
            $p1->SetColor('black');
        ...
        $total = 0;
        foreach ($data['incendi_boschivi_provincia'] as $row) $total += $row['conteggio'];
        if($total > 0) {
            $graph = new Graph\PieGraph(400, 300);
            ...
            $pie_data = array_map(function($e){ return $e['conteggio'];}, $data['incendi_boschivi_provincia']);
            $p1 = new Plot\PiePlot($pie_data);
            $p1->value->setFormat('%01.2f%%');
            $p1->ShowBorder();
            $p1->SetColor('black');
