...
class SegnalazioneController extends Controller
{
    ...
    public function actionView($id)
    {
        ...
        //if($model->tipologia) :
            /*
                ...
                ->where(['tipologia_evento' => $model->tipologia->id])
                ->orderBy('dataora_evento DESC')
                ->orderBy('ST_Distance()')
                ->limit(10)
                ->all();
            */
            ...
            ->select( [
                '*', 
                'ST_DistanceSphere(geom, ST_MakePoint(:lon, :lat)) as list_distance',
    ...
    public function actionChangeEvento($id)
    {
        ...
        try {
            ...
            if(empty($model->tipologia_evento)) throw new \Exception("Inserisci la tipologia di segnalazione", 1);
    ...
    public function actionCreate()
    {
        ...
        if(Yii::$app->request->get('from_cap')) $model->loadDataFromCapMessage(Yii::$app->request->get('from_cap'));
    ...
    public function actionChangeEventoAssociato( $id_segnalazione, $id_nuovo_evento )
    {
        ...
        try {
            ...
            if(count($connection) != 1) throw new \Exception("La segnalazione non è associata a un singolo evento, verifica la connessione in con_evento_segnalazione where idsegnalazione = " . $id_segnalazione, 1);
